...
export function ShoppingCart({ cart }: { cart: ShoppingCartPageView }) {
  ...
  return <section className="space-y-6">
    ...
    {cart.items.length ? <Card><CardHeader><CardTitle>Your list</CardTitle><CardDescription>Checkoffs stay on this page only and reset when you return.</CardDescription></CardHeader><CardContent className="space-y-2">{cart.items.map((item) => {
      ...
      return <button key={item.itemId} type="button" onClick={() => setChecked((current) => { const next = new Set(current); if (next.has(item.itemId)) next.delete(item.itemId); else next.add(item.itemId); return next; })} className="flex w-full items-start gap-3 rounded-xl border border-border/70 p-3 text-left transition hover:bg-secondary/30">
        ...
        <span className="min-w-0 flex-1"><span className={`block font-medium ${isChecked ? "text-muted-foreground line-through" : ""}`}>{quantity ? `${quantity} ${item.displayName}` : item.displayName}</span><span className="mt-0.5 block text-xs text-muted-foreground">{item.sourceMeals.map((meal) => `${meal.recipeTitle} (${meal.date})`).join(" · ")}</span></span>
      </button>;
    ...
    <Dialog open={alwaysOpen} onOpenChange={setAlwaysOpen}><DialogContent><DialogHeader><DialogTitle>Always haves</DialogTitle><DialogDescription>Enabled ingredients are excluded from new shopping lists. Turn one off when you need to buy it.</DialogDescription></DialogHeader><div className="space-y-4">
      {cartIngredients.length ? <div><p className="mb-2 text-sm font-medium">Add from this cart</p><div className="space-y-2">{cartIngredients.map((item) => <div className="flex items-center justify-between gap-3" key={item.canonicalIngredientId}><span className="text-sm">{item.displayName}</span><Button size="sm" variant="outline" disabled={isPending} onClick={() => { const data = new FormData(); data.set("canonicalIngredientId", item.canonicalIngredientId); data.set("dates", JSON.stringify(cart.selectedDates)); runAction(addAlwaysHaveIngredientAction, data); }}>Add</Button></div>)}</div></div> : null}
      <div><p className="mb-2 text-sm font-medium">Saved ingredients</p>{cart.alwaysHaves.length ? <div className="space-y-3">{cart.alwaysHaves.map((item) => <div className="flex items-center gap-3" key={item.canonicalIngredientId}><Switch checked={item.enabled} disabled={isPending} onCheckedChange={(enabled) => { const data = new FormData(); data.set("canonicalIngredientId", item.canonicalIngredientId); data.set("enabled", String(enabled)); runAction(setAlwaysHaveIngredientEnabledAction as typeof addAlwaysHaveIngredientAction, data); }} /><span className="flex-1 text-sm">{item.displayName}</span><Button size="icon" variant="ghost" aria-label={`Remove ${item.displayName}`} disabled={isPending} onClick={() => { const data = new FormData(); data.set("canonicalIngredientId", item.canonicalIngredientId); runAction(removeAlwaysHaveIngredientAction as typeof addAlwaysHaveIngredientAction, data); }}><Trash2 className="h-4 w-4" /></Button></div>)}</div> : <p className="text-sm text-muted-foreground">No saved always haves.</p>}</div>
