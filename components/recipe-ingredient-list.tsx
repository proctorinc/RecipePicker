...
export function RecipeIngredientList({
  ...
  const [reviewingIngredient, setReviewingIngredient] = useState<
    IngredientListProps["ingredients"][number] | null
  >(null);
  const router = useRouter();
  const [, startTransition] = useTransition();

  const reviewItem: IngredientReviewItemView | null =
    reviewingIngredient && recipeId && recipeTitle
      ? {
          ingredientId: reviewingIngredient.id,
          recipeId,
          recipeTitle,
          originalText: reviewingIngredient.originalText,
          measurements: (reviewingIngredient.measurements ?? []).map(
            (measurement, index) => ({
              id: `recipe-measurement-${index}`,
              ...measurement,
            }),
          ),
  ...
  return (
    <div>
        ...
        {ingredients.map((ingredient) => {
          ...
          return (
            ...
            >
              ...
              {(() => {
                const parts = formatScaledIngredientParts(
                  ingredient,
                  multiplier,
                );
                return parts.amount ? (
                  <span className="mt-0.5 shrink-0 rounded-lg bg-secondary px-2 py-1 text-sm font-semibold text-foreground">
                    {parts.amount}
                  </span>
                ) : null;
              })()}
              <div className="min-w-0 flex-1">
                <p className="pt-1 font-medium text-foreground">
                  {
                    formatScaledIngredientParts(ingredient, multiplier)
                      .description
                  }
                </p>
      ...
      <IngredientReviewDialog
        ...
        onOpenChange={(open) => {
          if (!open) setReviewingIngredient(null);
        }}
        onDone={() => {
