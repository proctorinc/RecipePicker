...
describe("formatScaledIngredient", () => {
  it("preserves the imported ingredient line at 1×", () => {
    expect(formatScaledIngredient(ingredient({ originalText: "1 tsp vanilla" }), 1)).toBe("1 tsp vanilla");
  });

  it("scales amounts and promotes teaspoons to tablespoons", () => {
    expect(formatScaledIngredient(ingredient(), 3)).toBe("1 tablespoon vanilla extract");
    expect(formatScaledIngredient(ingredient({ amountValue: 2, originalText: "2 tsp vanilla extract" }), 2)).toBe("1⅓ tablespoons vanilla extract");
  });
  ...
  it("promotes tablespoons to cups and formats common fractions", () => {
    ...
    expect(formatScaledIngredient(ingredient({ amountValue: 0.5, originalText: "1/2 teaspoon salt", parsedText: "salt" }), 3)).toBe("1½ teaspoons salt");
  ...
  it("scales both values in a range without changing to an inconsistent unit", () => {
    expect(formatScaledIngredient(ingredient({ originalText: "1-2 teaspoons pepper", amount: "1-2", amountMaxValue: 2, parsedText: "pepper" }), 2)).toBe("2–4 teaspoons pepper");
    expect(formatScaledIngredient(ingredient({ originalText: "1-2 teaspoons pepper", amount: "1-2", amountMaxValue: 2, parsedText: "pepper" }), 3)).toBe("1–2 tablespoons pepper");
  });

  it("keeps precise decimals, non-volume units, notes, and unparseable lines safe", () => {
    expect(formatScaledIngredient(ingredient({ originalText: "0.2 pound mushrooms", amountValue: 0.2, unit: "pound", parsedText: "mushrooms" }), 3)).toBe("0.6 pound mushrooms");
