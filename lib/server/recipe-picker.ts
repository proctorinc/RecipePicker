...
import { getHouseholdAiConnectionSummary, generateRecipePickerWithHouseholdAi } from "@/lib/server/ai-provider";
import { requireHouseholdContext } from "@/lib/server/auth";
import { openDatabase } from "@/lib/server/database";
import { resolveIngredientSearchQuery, normalizeIngredientKey } from "@/lib/server/ingredient-normalization";
import {
...
type RecipePickerRow = ReturnType<typeof getRecipePickerRows>[number];
type ConversationRow = ReturnType<typeof getConversationRows>[number];
...
export async function getRecipePickerConversationState(args: {
  ...
  try {
    const conversations = getConversationRows(db, args.householdId);
    const threadSummaries = conversations.map(toConversationSummary);
    const conversation = conversations.find((entry) => entry.conversationId === args.conversationId)
      ?? conversations[0]
      ?? null;
    ...
    if (!conversation) {
      const conversationId = await insertRecipePickerConversation(args);
      return {
        ...(await getRecipePickerConversationState({ ...args, conversationId })),
      };
    }

    const messages = conversation.messages.map((message) => toChatMessage(message));
    const activeAssistantMessage = [...messages].reverse().find((message) => message.role === "assistant") ?? null;
    ...
    return {
      ...
      intent: (activeAssistantMessage?.intent ?? "replace_set") as RecipePickerIntent,
      setExplanation: "",
...
export async function runRecipePicker(args: {
  ...
  const conversationId = request.conversationId
    ?? (await ensureRecipePickerConversation({ householdId: args.householdId, clerkUserId: args.clerkUserId }));
  ...
  try {
    const conversation = db.query.householdRecipePickerConversations.findFirst({
      where: (table, { eq: whereEq }) => and(whereEq(table.conversationId, conversationId), whereEq(table.householdId, args.householdId)),
      with: {
        messages: {
    ...
    const nextTitle = conversation.title ?? deriveConversationTitle(request.prompt);

    sqlite.transaction(() => {
      db.insert(householdRecipePickerMessages)
        .values([
          {
            conversationId,
            householdId: args.householdId,
          ...
          },
          {
            ...
            activeRecipeId: generated.recipes[generated.activeIndex]?.recipeId ?? null,
      ...
      db.update(householdRecipePickerConversations)
        .set({
          title: nextTitle,
          lastMessageAt: now,
          updatedAt: now,
        })
        .where(eq(householdRecipePickerConversations.conversationId, conversationId))
        .run();
    })();
  } finally {
    sqlite.close();
...
async function insertRecipePickerConversation(args: {
  ...
  try {
    const row = db.insert(householdRecipePickerConversations)
      .values({
        householdId: args.householdId,
      ...
      .returning({ conversationId: householdRecipePickerConversations.conversationId })
      .get();
...
async function ensureRecipePickerConversation(args: {
  ...
  try {
    const existing = db.query.householdRecipePickerConversations.findFirst({
...
function getConversationRows(db: ReturnType<typeof openDatabase>["db"], householdId: string) {
  return db.query.householdRecipePickerConversations.findMany({
...
async function generateRecipePickerTurn(args: {
  ...
  try {
    const recipes = buildRecipeCandidates(getRecipePickerRows(db, args.householdId));
    ...
    const includeIngredientResolutions = interpretation.mustIncludeIngredients
      ...
      .filter((value): value is NonNullable<IngredientResolution> => Boolean(value));
    ...
    const recipeCards = selected.map((recipe) => toRecipePickerCard(recipe, request.pinnedRecipeIds));
    ...
    return {
      ...
      recipes: recipeCards,
      pinnedRecipeIds: request.pinnedRecipeIds.filter((recipeId) => recipeCards.some((recipe) => recipe.recipeId === recipeId)),
...
async function interpretRecipePrompt(args: {
  ...
  const topRatedRecipes = [...args.recipeMap.values()]
    ...
    .slice(0, 10)
    .map((recipe) => {
      const notes = recipe.reviewNotes.slice(0, 2).map((note) => `"${trimForPrompt(note, 120)}"`).join("; ");
  ...
  const selectedRecipes = args.currentSetRecipeIds
    ...
    .join("\n");
  const activeRecipe = args.activeRecipeId ? args.recipeMap.get(args.activeRecipeId) : null;
  const prompt = [
    "You help choose recipes from an existing household recipe catalog.",
    "Return JSON only.",
    ...
    "",
    activeRecipe ? `Active recipe: ${activeRecipe.recipeId} | ${activeRecipe.title}` : "Active recipe: none",
    orderedCurrentRecipes ? `Current set:\n${orderedCurrentRecipes}` : "Current set: none",
    selectedRecipes ? `Recipes available for inline mention:\n${selectedRecipes}` : "Recipes available for inline mention: none",
    pinnedRecipes ? `Pinned recipes:\n${pinnedRecipes}` : "Pinned recipes: none",
    topRatedRecipes ? `Top reviewed household recipes:\n${topRatedRecipes}` : "Top reviewed household recipes: none",
  ].join("\n");
...
function getRecipePickerRows(db: ReturnType<typeof openDatabase>["db"], householdId: string) {
...
function buildRecipeCandidates(rows: RecipePickerRow[]): RecipeCandidate[] {
  return rows.map((row) => {
    ...
    const ingredients = (row.recipeInstructions?.ingredients ?? []).map((ingredient) => ({
    ...
    const title = row.title ?? row.recipeInstructions?.title ?? row.pin.title ?? "Untitled recipe";
    const shortDescription = row.description ?? row.recipeInstructions?.description ?? row.pin.description ?? null;
    const imageUrl = row.imageUrl ?? row.recipeInstructions?.imageUrl ?? getPinImageUrl(row.pin.mediaJson, row.pin.rawJson);
    ...
    return {
      ...
      timeText: [row.recipeInstructions?.prepTime, row.recipeInstructions?.cookTime, row.recipeInstructions?.totalTime].filter(Boolean).join(" "),
...
function normalizeInterpretation(raw: z.infer<typeof pickerAiSchema>): PickerInterpretation {
  return {
    intent: raw.intent,
...
export function fallbackInterpretRecipePrompt(args: {
  ...
  if (!prompt) {
    return {
      ...
      similarToCurrentRecipeIds: args.activeRecipeId ? [args.activeRecipeId] : [],
      reviewTerms: [],
      preferHighlyRated: false,
      preferRecipesYouLike: false,
      assistantMessage: "Here are a few recipes you can start with from your saved collection.",
      followUpQuestions: [
        "Show me cozy pasta or skillet dishes",
        "I want something lighter with vegetables",
        "Give me quick dinners under 30 minutes",
      ],
      explanation: "Starting with a broad mix of recipes from your household collection.",
    };
  }
  ...
  const multiWordTerms = [
    ...[...FALLBACK_MEAL_TYPES, ...FALLBACK_KEYWORDS, ...FALLBACK_CUISINES].filter((term) => prompt.includes(term)),
  ];
  const mealTypes = multiWordTerms.filter((term) => FALLBACK_MEAL_TYPES.has(term));
  const keywordTerms = multiWordTerms.filter((term) => FALLBACK_KEYWORDS.has(term));
  const cuisineTerms = multiWordTerms.filter((term) => FALLBACK_CUISINES.has(term));

  const intent: RecipePickerIntent = /\b(add|also|another|more)\b/.test(prompt) && /\bkeep\b/.test(prompt)
    ? "add_to_set"
  ...
  const recipeTitleMatches = [...args.recipeMap.values()]
    .filter((recipe) => prompt.includes(recipe.title.toLowerCase()) || (recipe.pinTitle && prompt.includes(recipe.pinTitle.toLowerCase())))
    .map((recipe) => recipe.recipeId);
  const reviewFocusedRecipeIds = [...args.recipeMap.values()]
    .filter((recipe) => recipe.reviewCount > 0)
    .filter((recipe) => recipe.reviewNotes.some((note) => prompt.includes(note.toLowerCase())))
    .map((recipe) => recipe.recipeId);
  const ingredientCandidates = terms.filter((term) => !STOP_WORDS.has(term) && term.length > 2);
  const titleTerms = ingredientCandidates.filter((term) => /\b(chow|mein|pasta|cookie|salad|fajita|orzo)\b/.test(term));
  const preferHighlyRated = /\b(best rated|top rated|highest rated|best-reviewed|most loved|favorite|favorites)\b/.test(prompt);
  const preferRecipesYouLike = /\b(really like|i like|we like|love|favorites?|go-to|go to)\b/.test(prompt);
  const reviewTerms = uniqueStrings(
    ingredientCandidates.filter((term) => !titleTerms.includes(term)).filter((term) =>
      /\b(review|reviews|rated|rating|favorite|favorites|love|loved|crispy|creamy|spicy|tender|juicy|saucy|comfort)\b/.test(term),
    ),
  );
  const mustIncludeIngredients = ingredientCandidates.filter((term) =>
  ...
  return {
    ...
    explanation: preferHighlyRated || preferRecipesYouLike || reviewTerms.length > 0
...
function scoreRecipeCandidate(recipe: RecipeCandidate, context: ScoreContext): RecipeCandidate {
  const reasons: string[] = [];
  let score = recipe.reviewCount * 0.4 + (recipe.averageRating ?? 0) * 0.35;

  if (context.interpretation.preferHighlyRated || context.interpretation.preferRecipesYouLike) {
    score += getReviewPreferenceBoost(recipe);
    if (recipe.reviewCount > 0 && recipe.averageRating !== null) {
      reasons.push(`Rated ${recipe.averageRating.toFixed(1)} stars by your household`);
    }
  }
  ...
  for (const term of context.interpretation.titleTerms) {
    if (termMatches(recipe.title.toLowerCase(), term) || (recipe.pinTitle && termMatches(recipe.pinTitle.toLowerCase(), term))) {
      score += 5;
  ...
  if (context.interpretation.timeHint && matchesTimeHint(recipe.timeText, context.interpretation.timeHint)) {
    score += 2;
  ...
  if (context.currentSetRecipeIds.includes(recipe.recipeId) && context.interpretation.intent !== "replace_set") {
    score += 1.5;
  }
...
function buildRecipePickerSet(args: {
  ...
  const candidateMap = new Map(args.candidates.map((recipe) => [recipe.recipeId, recipe]));
  ...
  if (args.intent === "add_to_set") {
    for (const recipeId of args.currentSetRecipeIds) {
      ...
      if (recipe && !pinnedIds.has(recipe.recipeId) && selected.length < TARGET_SET_SIZE) {
  ...
  if (args.intent === "refine_set") {
    const existingRanked = args.currentSetRecipeIds
      .map((recipeId) => candidateMap.get(recipeId))
      .filter((value): value is RecipeCandidate => Boolean(value))
      .filter((recipe) => !selected.some((entry) => entry.recipeId === recipe.recipeId))
      .sort(compareRankedRecipes);
    for (const recipe of existingRanked) {
      if ((recipe.isStrongMatch || selected.length < MIN_SET_SIZE) && selected.length < TARGET_SET_SIZE) {
        selected.push(recipe);
      }
    }
  }
...
function buildAssistantMessage(args: {
  ...
  const referenced = args.recipeCards.slice(0, Math.min(2, args.recipeCards.length))
    .map((recipe) => `<recipe:${recipe.recipeId}|${recipe.title}>`);
  const referenceText = referenced.length > 0 ? ` I’d start with ${referenced.join(" and ")}.` : "";
  ...
  const normalizedMessage = args.interpretation.assistantMessage || "I pulled together a set that fits what you asked for.";
  return normalizedMessage.includes("<recipe:") ? normalizedMessage : `${normalizedMessage}${referenceText}`;
}
...
function resolveActiveIndex(selected: RecipeCandidate[], activeRecipeId: string | null | undefined) {
  if (!activeRecipeId) {
    return 0;
  }

  const index = selected.findIndex((recipe) => recipe.recipeId === activeRecipeId);
  return index >= 0 ? index : 0;
}

function resolveActiveIndexFromCards(selected: RecipePickerCard[], activeRecipeId: string | null | undefined) {
  if (!activeRecipeId) {
    return 0;
  }
...
function toRecipePickerCard(recipe: RecipeCandidate, pinnedRecipeIds: string[]): RecipePickerCard {
  return {
    ...
    matchedReasons: recipe.matchedReasons.length > 0 ? recipe.matchedReasons : ["Broader match from your saved recipes"],
...
function toChatMessage(message: ConversationMessageRow): RecipePickerChatMessage {
  const inlineRecipeRefs = safeParseJson<RecipePickerInlineRecipeRef[]>(message.inlineRecipeRefsJson, []);
  const parsedMessage = parseStoredInlineRecipeReferences(message.bodyText, inlineRecipeRefs);
  ...
  return {
    ...
    recipeSnapshot: safeParseJson<RecipePickerCard[] | null>(message.recipeSnapshotJson, null),
...
function toConversationSummary(conversation: ConversationRow): RecipePickerConversationSummary {
...
function parseInlineRecipeReferences(
  bodyText: string,
  allowedRecipes: Map<string, RecipePickerCard>,
  ...
  while ((match = regex.exec(bodyText)) !== null) {
    ...
    if (match.index > cursor) {
      segments.push({ type: "text", text: bodyText.slice(cursor, match.index) });
    }
...
function parseStoredInlineRecipeReferences(
  ...
  const allowedLabels = new Map(storedRefs.map((ref) => [`${ref.recipeId}|${ref.label}`, ref]));
  const regex = /<recipe:([^|>]+)\|([^>]+)>/g;
  const segments: RecipePickerMessageSegment[] = [];
  let cursor = 0;
  let match: RegExpExecArray | null;
  ...
  while ((match = regex.exec(bodyText)) !== null) {
    ...
    if (stored) {
      segments.push({ type: "recipe", recipeId: stored.recipeId, label: stored.label });
    } else {
      segments.push({ type: "text", text: raw });
    }
...
function findIngredientResolutionMatch(recipe: RecipeCandidate, resolution: NonNullable<IngredientResolution>) {
  let bestScore = 0;
  let bestReason = `Includes ${resolution.canonicalDisplayName}`;

  for (const ingredient of recipe.ingredients) {
    if (!ingredient.canonicalIngredientId || !resolution.searchCanonicalIngredientIds.includes(ingredient.canonicalIngredientId)) {
      continue;
    }

    const attributeMatch = resolution.attributes.every((attribute) => ingredient.attributes.includes(attribute));
    const score = ingredient.canonicalIngredientId === resolution.canonicalIngredientId
    ...
    if (score > bestScore) {
      bestScore = score;
      bestReason = resolution.attributes.length > 0
...
function matchesTimeHint(timeText: string, timeHint: string) {
  ...
  if (!recipeMinutes || !requestedMinutes) {
    return normalizeIngredientKey(timeText).includes(normalizeIngredientKey(timeHint));
  }

  return Math.abs(recipeMinutes - requestedMinutes) <= 15 || recipeMinutes <= requestedMinutes + 10;
}
...
function tokenizeSimilarityText(recipe: RecipeCandidate) {
  return [
    ...
    recipe.cuisine ?? "",
    recipe.ingredients.map((ingredient) => ingredient.canonicalName ?? ingredient.ingredientText ?? ingredient.originalText).join(" "),
  ]
    .join(" ")
    .toLowerCase()
    .split(/[^a-z0-9]+/g)
...
function compareRecipesByPreferenceSignal(left: RecipeCandidate, right: RecipeCandidate) {
...
function buildFallbackFollowUpQuestions(args: {
  ...
  if (args.mealTypes.length > 0 || args.cuisineTerms.length > 0 || args.keywordTerms.length > 0 || args.reviewTerms.length > 0) {
    return [
...
function isGeneralPrompt(prompt: string) {
  ...
  return (
    ...
    !/\b(chicken|beef|pork|shrimp|salad|pasta|soup|taco|bowl|sandwich|curry|noodle|dessert|breakfast|lunch|dinner)\b/.test(normalized)
  );
}
